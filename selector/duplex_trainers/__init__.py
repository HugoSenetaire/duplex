...
Now you can use the trainer class by specifying flag '--trainer_trainer dummy'.
...
def find_trainer_using_name(trainer_name):
    """Import the module "duplex_trainers/[trainer_name]_trainer.py".

    In the file, the class called DatasetNametrainer() will
    be instantiated. It has to be a subclass of Basetrainer,
    and it is case-insensitive.
    """
    trainer_filename = "duplex_trainers." + trainer_name + "_trainer"
    ...
    for name, cls in trainerlib.__dict__.items():
        if name.lower() == target_trainer_name.lower() \
           and issubclass(cls, BaseSelectorTrainer):
            trainer = cls
